...
class Movement:
    ...
    def should_use_super_on_enemy(brawler, super_type, enemy_distance, attack_range, super_range, enemy_hittable):
        """Single source of truth for "is this a sane moment to fire the super?".

        Strict ``super_range`` gate: if the enemy is past super_range we'd
        rather have the bot's normal approach logic close the gap and fire
        the super in-range than waste it as a short fall-shot. The ``stu``/
        ``surge`` exception is preserved because their charge supers are
        intentionally long-range. ``spawnable``/``other`` supers don't need a
...
class Play(Movement):
    ...
    def _compute_combat_teammate_pull(self, teammate_distance):
        """Continuous teammate-pull strength for the Showdown combat path.

        Returns (mode, pull) where mode ∈ {"none","blend","panic"} and pull is
        the blend strength to pass to ``blend_angles`` (0..bias).

        - distance ≤ teammate_follow_max_distance: no pull (stay focused on the fight)
        - distance > max_follow .. < panic: linear ramp 0 → teammate_combat_bias
        - distance ≥ panic (regroup × 1.4): override desired with team_angle
        """
        max_follow = max(1.0, float(self.teammate_follow_max_distance))
        regroup = max(max_follow + 1.0, float(self.teammate_combat_regroup_distance))
        panic = regroup * 1.4
        if teammate_distance >= panic:
            return "panic", 1.0
        if teammate_distance > max_follow:
            ramp = (teammate_distance - max_follow) / max(1.0, regroup - max_follow)
            ramp = max(0.0, min(1.0, ramp))
            pull = ramp * float(self.teammate_combat_bias)
            if pull > 0.01:
                return "blend", pull
        return "none", 0.0
    ...
    def get_showdown_movement(self, player_data, enemy_data, teammate_data, walls, brawler):
        ...
        if not self.is_there_enemy(enemy_data):
            ...
            # Showdown prefers an actually-reachable enemy: if every visible
            # enemy is behind a wall we'd rather regroup with our teammate than
            # chase an unhittable target across the map.
            enemy_coords, enemy_distance = self.find_closest_enemy(
                enemy_data, player_pos, walls, "attack", prefer_hittable_only=True
            )
                ...
                if self.trio_grouping_enabled and teammate_data:
                    ...
                    if closest_teammate is not None:
                        ...
                        if mode != "none":
                            ...
                            if mode == "panic":
                                desired = team_angle
                                vlog(f"combat panic regroup → desired={desired:.1f}° (team dist={int(teammate_distance)}px)")
                            else:  # "blend"
                                desired = self.blend_angles(desired, team_angle, pull)
                                vlog(f"combat regroup bias → desired={desired:.1f}° (team dist={int(teammate_distance)}px, pull={pull:.2f})")
        ...
        if self.is_super_ready and self.time_since_holding_attack is None:
            ...
            if self.should_use_super_on_enemy(
    ...
    def find_closest_enemy(self, enemy_data, player_coords, walls, skill_type, prefer_hittable_only=False):
        ...
        if prefer_hittable_only:
            # Showdown: when no enemy is hittable, callers want to fall back to
            # teammate-follow rather than chase an enemy through walls.
            return None, None
    ...
    def _ability_threshold(self, base_threshold: float) -> float:
        """Scale a pixel-count threshold by the current frame area ratio.

        Pixel-count thresholds in cfg/bot_config.toml (super_pixels_minimum
        etc.) were originally calibrated against the full 1920×1080 reference
        frame. When scrcpy compresses to ``scrcpy_max_width = 960``, the
        actual decoded frames are ~960×540 — the same ROI rect contains 1/4
        of the pixels, so the count of "yellow"/"green"/"purple" pixels also
        drops to ~1/4. Without this scaling the bot literally CANNOT trigger
        is_*_ready on smaller scrcpy outputs (live multi-instance test on
        Colt logged 917 super checks across 2 wins, 0 above the threshold —
        the bot won the matches without ever firing E).
        """
        wr = self.window_controller.width_ratio or 1.0
        hr = self.window_controller.height_ratio or 1.0
        # Floor the scaling at 0.25 so a stray pre-init NoneType -> 1.0 value
        # doesn't make the threshold absurd; cap at 1.0 to keep the original
        # behaviour for users that already run at 1920×1080 native.
        return base_threshold * max(0.25, min(1.0, wr * hr))
    ...
    def check_if_hypercharge_ready(self, frame):
        ...
        threshold = self._ability_threshold(self.hypercharge_pixels_minimum)
        if debug:
            print(f"hypercharge purple pixels: {purple_pixels} (if > {threshold:.0f} then hypercharge is ready)")
    ...
    def check_if_gadget_ready(self, frame):
        ...
        threshold = self._ability_threshold(self.gadget_pixels_minimum)
        if debug:
            print(f"gadget green pixels: {green_pixels} (if > {threshold:.0f} then gadget is ready)")
    ...
    def check_if_super_ready(self, frame):
        wr, hr = self.window_controller.width_ratio, self.window_controller.height_ratio
        # Production ROI is a 100×100 box around where Brawl Stars used to draw
        # the super button. Newer game updates have nudged the button by a few
        # tens of pixels, so the configured ROI sometimes misses the yellow
        # entirely. Pad the ROI by ~80px on each side; the pad is still well
        # away from the gadget/hypercharge yellows below it (different HSV
        # range anyway).
        pad = int(80 * min(wr, hr))
        h, w = frame.shape[:2]
        x1 = max(0, int(self.super_crop_area[0] * wr) - pad)
        y1 = max(0, int(self.super_crop_area[1] * hr) - pad)
        x2 = min(w, int(self.super_crop_area[2] * wr) + pad)
        y2 = min(h, int(self.super_crop_area[3] * hr) + pad)
        screenshot = frame[y1:y2, x1:x2]
        yellow_pixels = count_hsv_pixels(screenshot, (17, 170, 200), (27, 255, 255))
        threshold = self._ability_threshold(self.super_pixels_minimum)
        if debug:
            print(f"super yellow pixels: {yellow_pixels} (if > {threshold:.0f} then super is ready)")
    ...
    def main(self, frame, brawler, main):
        ...
        if not data:
            ...
            return
        self.time_since_last_proceeding = time.time()
        # NOTE: do NOT reset readiness flags to False each frame. The throttle
        # window is ~250ms; at high IPS that would force False on the majority
        # of frames between checks, slashing super/gadget/hypercharge fire rate.
        # The fire branches (use_super/use_gadget/use_hypercharge sites in loop)
        # explicitly set the flag to False AND bump the corresponding _checked
        # timestamp, which holds the gate closed long enough for the on-screen
        # button to drain before the next check picks up the new state.

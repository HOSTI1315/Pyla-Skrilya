...
import requests
from utils import load_toml_as_dict, save_dict_as_toml, api_base_url, match_log_path

TROPHY_LOG_DIR = "logs"
TROPHY_LOG_FILE = "trophies.log"
TROPHY_LOG_MAX_LINES = 1000
MATCH_LOG_MAX_LINES = 3000
MILESTONE_STEP = 100
class TrophyObserver:
    ...
    def _append_match_log_entry(self, brawler, game_result, bucket, delta):
        """Append one JSON line to cfg/match_log.jsonl.

        Consumed by /api/stats to build per-brawler trophy totals and the
        "last matches" list in the web UI.
        """
        entry = {
            "ts": int(time.time()),
            "brawler": brawler,
        ...
        # Resolve at call time so a per-instance bot subprocess writes into
        # its own ``instances/<id>/cfg/match_log.jsonl`` instead of the
        # shared global path.
        path = match_log_path()
        try:
            ...
            self._trim_match_log()
        except OSError as e:
            print(f"Failed to append match log: {e}")

    def _trim_match_log(self):
        path = match_log_path()
        try:
            ...
            with open(path, "w", encoding="utf-8") as f:
                f.writelines(lines[-MATCH_LOG_MAX_LINES:])
        except OSError as e:
            print(f"Failed to trim match log: {e}")
    ...
    def add_trophies(self, game_result, current_brawler):
        ...
        self._append_match_log_entry(
            current_brawler, game_result, bucket, self.current_trophies - old,
        )

...
class TestLobbyAutomation(unittest.TestCase):
    ...
    def test_can_select_brawlers(self, mock_extract_text):
        ...
        # select_brawler now taps the OCR label center directly (no -95px hack
        # over the icon), so the expected click lands on the label of the card.
        expected_brawler_x = 2012
        expected_brawler_y = 1072
        tolerance = 50

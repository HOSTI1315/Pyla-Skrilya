...
def get_optimal_threads(max_limit=4):
    ...
    # Auto-scale the cap with the host: the historical max_limit=4 made sense
    # on a single-instance laptop bot, but starves high-core servers like a
    # dual Xeon (56t) running CPU-only inference. For >=32 logical cores raise
    # to 8 so a single instance can actually hit a reasonable IPS; multi-
    # emulator setups should still set ``used_threads`` explicitly to divide
    # the budget across instances.
    if max_limit == 4 and threads >= 32:
        max_limit = 8
    threads_amount = min(max(2, threads // 2), max_limit)
    print(f"Detected {threads} CPU threads, using {threads_amount} threads (max_limit={max_limit}).")
    return threads_amount
